...
} from '@infraestructure/helpers/control-error.directive';
...
describe('NgxControlError', () => {
  ...
  const render = (
    template: string,
    inputs?: Partial<NgxControlErrorDirective> | undefined,
    providers?: Provider[]
  ) => {
    @Component({
      imports: [CommonModule, NgxControlErrorDirective],
      standalone: true,
      template,
      providers,
    })
    ...
    const controlError = fixture.debugElement
      .queryAllNodes(By.directive(NgxControlErrorDirective))[0]
      .injector.get(NgxControlErrorDirective);
  ...
  it(
    'should be created',
    unitTest(() => expect(new NgxControlErrorDirective()).toBeTruthy())
  );
  ...
  it(
    ...
    unitTest(() =>
      expect(
        NgxControlErrorDirective.ngTemplateContextGuard(
          new NgxControlErrorDirective(),
          {}
        )
      ).toBe(true)
    )
  ...
  describe('should have an error when the control includes the tracked error and the control is in an error state', () => {
    it(
      ...
      unitTest(() => {
        const instance = new NgxControlErrorDirective();
